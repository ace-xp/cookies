  ...
  return <div className="video-editing-workspace c3-editor-workspace">
    ...
    <div className="c3-editor-shell">
      ...
      <aside className="c3-inspector">
        ...
        <div className="c3-track-inspector"><b>音频总线</b>{document.audio.tracks.map(track => <div key={track.id} className={`c3-track-state${activeAudioTrackId === track.id ? ' active' : ''}`} role="button" tabIndex={0} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setActiveAudioTrackId(track.id); setLeftMode('audio') } }} onClick={() => { setActiveAudioTrackId(track.id); setLeftMode('audio') }}><span>{audioRoleLabel(track.role)} · {track.clips.length} 段</span><button title={track.muted ? '取消静音' : '静音轨道'} onClick={event => { event.stopPropagation(); commit(current => ({ ...current, audio: setAudioTrackMuted(current.audio, track.id, !track.muted) })) }}>{track.muted ? <VolumeX size={13}/> : <Volume2 size={13}/>}</button></div>)}</div>
