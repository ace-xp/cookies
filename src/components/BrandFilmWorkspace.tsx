...
import { api, type ApiAssetVersionRef, type ApiBrandAudioMixOperation, type ApiBrandAudioWorkspace, type ApiBrandBriefAnalysis, type ApiBrandBriefAssetCandidate, type ApiBrandCreativeConcept, type ApiBrandFilmPlan, type ApiBrandFilmWorkspace, type ApiCreativeTaskSummary, type ApiSpeechCapability } from '../data/api'
import { BrandFilmWorkbenchShell } from '../features/brand-film/BrandFilmWorkbenchShell'
import { stringListOrEmpty } from '../features/brand-film/brand-film-value'
...
import { BrandFilmReviewTimeline } from '../features/brand-film/review-timeline/BrandFilmReviewTimeline'
...
function ConceptCard({ concept, editing, busy, onChange, onSelect }: {
  ...
  return <article className={concept.selected ? 'selected' : ''}><span>{concept.id}</span><h4>{concept.title}</h4><b>{concept.one_liner}</b><p>{concept.story_mechanism}</p><dl><div><dt>品牌进入</dt><dd>{concept.brand_entrance}</dd></div><div><dt>视觉语言</dt><dd>{stringListOrEmpty(concept.visual_language).join('、')}</dd></div><div><dt>声音</dt><dd>{concept.sound_idea}</dd></div><div><dt>Brief 依据</dt><dd>{concept.brief_rationale}</dd></div><div><dt>风险</dt><dd>{concept.risk}</dd></div></dl><button className={concept.selected ? 'secondary-button' : 'primary-button'} disabled={busy || concept.selected} onClick={onSelect}>{concept.selected ? '已选择' : '选择此方向'}</button></article>
}

function LegacyShotEditor({ shot, disabled, onChange }: { shot: ApiBrandFilmPlan['shots'][number]; disabled: boolean; onChange: (changes: Partial<ApiBrandFilmPlan['shots'][number]>) => void }) {
...
function StoryboardEditor({ plan, disabled, onChange }: { plan: ApiBrandFilmPlan; disabled: boolean; onChange: (plan: ApiBrandFilmPlan) => void }) {
  ...
  return <div className="brand-storyboard-editor">
    <SoundDesignIntentEditor plan={plan} disabled={disabled} onChange={changes => onChange({ ...plan, ...changes })}/>
...
function brandAudioProgress(audio: ApiBrandAudioWorkspace) {
  if (audio.mixed_preview_asset_ref || audio.status === 'preview_ready') return { title: 'Audio A2 混音预览已就绪', next: '可继续微调音轨或切换 A/B 声音方案', detail: '当前完整视频已经包含音乐、环境氛围与镜头音效；修改声音不会重新生成画面。' }
  ...
  if (brandAudioMaterialized(audio)) return { title: 'Audio A1 项目资产已就绪', next: '下一步：A2 FFmpeg 混音预览', detail: '音乐、环境氛围与镜头音效均可试听和替换；当前 Fixture 会被明确标记。' }
  return { title: 'Audio A0 草稿已保存', next: '下一步：将 Fixture 物化为项目音频资产', detail: '结构已持久化，尚未产生可试听音频。' }
}

function LegacyAudioWorkspaceEditor({ audio, videoURL, mixedVideoURL, assetURLs, speechCapability, busy, onMaterialize, onReplace, onSave, onRender, onProbeSpeech, onGenerateVoice, onSelectVariant, onPlanDirector }: {
...
export function BrandFilmWorkspace({ taskId, taskOptions, onOpenTask, onCreateNew, onNotice }: Props) {
  ...
  const materializeAudio = () => commit('materialize-audio', () => api.materializeBrandFilmAudioAssets(currentProject.id, workspace.task.id, revision), '开发演示音频已真实入库，现在可以逐段试听。')
  const generateSoundAssets = () => commit('generate-sound-assets', () => api.generateBrandFilmSoundAssets(currentProject.id, workspace.task.id, revision), 'AI 声音资产已生成并入库，现在可以逐段试听。')
  ...
  >
      ...
      {activeStage === 'brief' ? <section className="brand-film-section"><header><div><span className="section-label">PHASE 01</span><h3>Brief 分析与事实确认</h3></div><ModelBadge alias={brief?.model_alias} version={brief?.model_version}/></header>
        {!brief ? <div className="brand-film-empty"><FileText size={24}/><p>使用 Seed-2-pro 解析固定娇兰 Brief；不可用时回退固定样例。</p><button className="primary-button" disabled={Boolean(busy)} onClick={() => void analyze()}><WandSparkles size={15}/>解析 Brief</button></div> : <>
          <div className="brand-form-grid"><label className="wide">Brief 摘要<textarea disabled={lockedBrief} value={brief.summary} onChange={event => setBrief({ ...brief, summary: event.target.value })}/></label><label>目标人群<textarea disabled={lockedBrief} value={brief.audience} onChange={event => setBrief({ ...brief, audience: event.target.value })}/></label><label>核心传播信息<textarea disabled={lockedBrief} value={brief.core_message} onChange={event => setBrief({ ...brief, core_message: event.target.value })}/></label></div>
...
function ShotEditor({ shot, disabled, onChange }: { shot: ApiBrandFilmPlan['shots'][number]; disabled: boolean; onChange: (changes: Partial<ApiBrandFilmPlan['shots'][number]>) => void }) {
  return <article className="brand-shot-editor"><header><div><b>镜头 {String(shot.order).padStart(2, '0')}</b><span>{shot.start_second}s–{shot.end_second}s</span></div><small>{shot.purpose}</small></header><div className="brand-shot-editor-layout"><div className="brand-shot-canvas-fields"><label>画面<textarea disabled={disabled} value={shot.visual} onChange={event => onChange({ visual: event.target.value })}/></label><label>动作<textarea disabled={disabled} value={shot.action} onChange={event => onChange({ action: event.target.value })}/></label><label>屏幕字<textarea disabled={disabled} value={shot.on_screen_text} onChange={event => onChange({ on_screen_text: event.target.value })}/></label></div><aside className="brand-shot-inspector" aria-label={`镜头 ${shot.order} 属性`}><h4>镜头属性</h4><label>镜头目的<input disabled={disabled} value={shot.purpose} onChange={event => onChange({ purpose: event.target.value })}/></label><label>参考角色<input disabled={disabled} value={shot.reference_role} onChange={event => onChange({ reference_role: event.target.value })}/></label><label>运镜<textarea disabled={disabled} value={shot.camera} onChange={event => onChange({ camera: event.target.value })}/></label><label>光线<textarea disabled={disabled} value={shot.lighting} onChange={event => onChange({ lighting: event.target.value })}/></label><label>连贯性<textarea disabled={disabled} value={shot.continuity_notes} onChange={event => onChange({ continuity_notes: event.target.value })}/></label></aside></div></article>
}

function AudioWorkspaceEditor({ audio, videoURL, mixedVideoURL, assetURLs, busy, onMaterialize, onGenerateAI, onSave, onRender, onSelectVariant, onPlanDirector }: {
  audio: ApiBrandAudioWorkspace
  videoURL?: string
  mixedVideoURL?: string
  assetURLs: Record<string, string>
  ...
  const visibleTracks = (mix?.tracks ?? []).filter(track => track.type !== 'voiceover')
  const [selectedClipId, setSelectedClipId] = useState(() => visibleTracks.flatMap(track => track.clips)[0]?.id ?? '')
  const [trackDrafts, setTrackDrafts] = useState(() => Object.fromEntries(visibleTracks.map(track => [track.id, { gain: track.gain_db, muted: track.muted }])))
  const [clipDrafts, setClipDrafts] = useState(() => Object.fromEntries((visibleTracks.flatMap(track => track.clips) ?? []).map(clip => [clip.id, { label: clip.label, start: clip.timeline_start_ms, end: clip.timeline_end_ms, gain: clip.gain_db, fadeIn: clip.fade_in_ms, fadeOut: clip.fade_out_ms }])))
  if (!mix) return <p className="brand-locked">当前声音设计修订不可用，请重新准备声音方案。</p>
  const materialized = brandAudioMaterialized(audio)
  const blueprint = audio.blueprint_versions.at(-1)
  const clips = visibleTracks.flatMap(track => track.clips.map(clip => ({ track, clip })))
  const selected = clips.find(item => item.clip.id === selectedClipId) ?? clips[0]
  const operations: ApiBrandAudioMixOperation[] = []
  ...
  for (const { clip } of clips) {
    ...
    if (!draft) continue
    if (draft.label.trim() && draft.label !== clip.label) operations.push({ op: 'set_clip_label', clip_id: clip.id, label: draft.label.trim() })
    if (draft.start !== clip.timeline_start_ms || draft.end !== clip.timeline_end_ms) operations.push({ op: 'set_clip_timing', clip_id: clip.id, timeline_start_ms: draft.start, timeline_end_ms: draft.end })
    if (draft.gain !== clip.gain_db) operations.push({ op: 'set_clip_gain', clip_id: clip.id, gain_db: draft.gain })
    if (draft.fadeIn !== clip.fade_in_ms || draft.fadeOut !== clip.fade_out_ms) operations.push({ op: 'set_clip_fade', clip_id: clip.id, fade_in_ms: draft.fadeIn, fade_out_ms: draft.fadeOut })
  }
  const selectClip = (clipId: string) => {
    const target = clips.find(item => item.clip.id === clipId)
    setSelectedClipId(clipId)
    if (target && playerRef.current) {
      playerRef.current.currentTime = target.clip.timeline_start_ms / 1000
      void playerRef.current.play().catch(() => undefined)
    }
  }
  const currentDraft = selected ? clipDrafts[selected.clip.id] ?? { label: selected.clip.label, start: selected.clip.timeline_start_ms, end: selected.clip.timeline_end_ms, gain: selected.clip.gain_db, fadeIn: selected.clip.fade_in_ms, fadeOut: selected.clip.fade_out_ms } : undefined
  const updateCurrent = (changes: Partial<NonNullable<typeof currentDraft>>) => selected && setClipDrafts(value => ({ ...value, [selected.clip.id]: { ...currentDraft!, ...changes } }))
  const addCue = () => {
    const start = selected ? Math.min(audio.master_duration_ms - 600, selected.clip.timeline_end_ms) : 0
    const end = Math.min(audio.master_duration_ms, start + 600)
    onSave([{ op: 'add_fixture_clip', track_id: 'track_sfx', label: '新的镜头音效：请编辑声音描述', timeline_start_ms: start, timeline_end_ms: end }])
  }
  return <div className="brand-sound-design-workspace">
    <section className="brand-audio-director sound-design-summary">
      <header><div><span className="section-label">SOUND DESIGN · S0–S4</span><h4>AI 声音设计：音乐、环境氛围与镜头音效</h4><p>{audio.contract_version === 'creative-brand-audio-workspace/v1' ? '这是历史音轨版本：原有旁白资源被保留在任务修订历史中。创建新的声音设计版本后，当前任务将只管理音乐、环境氛围与镜头音效。' : '声音描述会发送给独立的 AI 音乐/音效模型；关键镜头音效会自动让品牌音乐让位，并由响度归一保证交付稳定。'}</p>{audio.contract_version === 'creative-brand-audio-workspace/v1' ? <button className="primary-button" disabled={busy} onClick={onPlanDirector}>创建无口播声音设计版本</button> : null}</div><div className="brand-audio-variants">{audio.variants.map(variant => <button key={variant.id} className={variant.id === audio.active_variant_id ? 'active' : ''} disabled={busy || variant.id === audio.active_variant_id} onClick={() => onSelectVariant(variant.id)}>{variant.label}<small>{variant.style_preset}</small></button>)}</div></header>
      <div className="brand-director-grid"><article><h5>声音意图</h5><b>{(blueprint?.sound_design_intent?.music_direction ?? blueprint?.music_arc.direction ?? '待确认音乐氛围').replaceAll('旁白', '主音效')}</b><small>音效重点：{stringListOrEmpty(blueprint?.sound_design_intent?.sound_effect_focus).join('、') || '镜头动作与产品材质'}</small><small>原视频声音：{blueprint?.sound_design_intent?.source_audio_policy === 'mute' ? '默认静音' : '可选使用'}</small></article><article><h5>声画检查</h5>{(blueprint?.semantic_checks ?? []).map(check => <div className={`brand-semantic-check ${check.status}`} key={check.id}><b>{check.status === 'pass' ? '通过' : '建议修复'} · {check.shot_id}</b><span>{check.summary.replaceAll('旁白', '声音事件').replaceAll('口播', '声音')}</span><small>{(check.suggestion || check.evidence).replaceAll('旁白', '声音事件').replaceAll('口播', '声音')}</small></div>)}</article></div>
    </section>
    <section className="brand-audio-preview sound-design-preview"><div>{mixedVideoURL ? <video ref={playerRef} controls src={mixedVideoURL}/> : videoURL ? <video ref={playerRef} controls muted src={videoURL}/> : <div className="brand-audio-placeholder"><Film size={24}/><span>视觉预览正在恢复</span></div>}</div><aside><span className="section-label">{mixedVideoURL ? 'MIXED PREVIEW' : materialized ? 'ASSET READY' : 'SOUND BLUEPRINT'}</span><b>{audio.variants.find(item => item.id === audio.active_variant_id)?.label ?? '默认声音方案'}</b><small>{audio.master_duration_ms / 1000} 秒 · Mix r{mix.revision}</small><p>{mixedVideoURL ? '正在播放画面与声音设计合成后的完整预览。修改任意声音事件后可重新混音，视觉画面保持不变。' : materialized ? '所有演示音频已经入库，可在时间轴中局部试听和编辑。' : '声音事件已自动排好，先生成可试听的演示音频后即可混音预览。'}</p></aside></section>
    <section className="brand-audio-timeline sound-design-timeline"><header className="sound-design-timeline-heading"><div><h5>声音时间轴</h5><small>点击声音块可跳到对应时刻；事件可以编辑描述、时间、音量与淡化。</small></div><button className="secondary-button" disabled={busy} onClick={addCue}><Plus size={14}/>添加镜头音效</button></header><div className="brand-audio-ruler"><span>00:00</span><span>{String(audio.master_duration_ms / 2000).padStart(2, '0')}s</span><span>{audio.master_duration_ms / 1000}s</span></div>{visibleTracks.map(track => { const draft = trackDrafts[track.id] ?? { gain: track.gain_db, muted: track.muted }; return <article key={track.id}><header><span>{draft.muted ? <VolumeX size={14}/> : <Volume2 size={14}/>}<b>{track.role}</b><small>{track.rights_status === 'fixture' ? '开发演示音频' : track.rights_status}</small></span><label>音量 <input type="range" min={-48} max={12} step={1} value={draft.gain} onChange={event => setTrackDrafts(value => ({ ...value, [track.id]: { ...draft, gain: Number(event.target.value) } }))}/><em>{draft.gain} dB</em></label><label className="brand-audio-mute"><input type="checkbox" checked={draft.muted} onChange={event => setTrackDrafts(value => ({ ...value, [track.id]: { ...draft, muted: event.target.checked } }))}/>静音</label></header><div className="brand-audio-lane">{track.clips.map(clip => <button type="button" key={clip.id} className={clip.id === selected?.clip.id ? 'selected' : ''} style={{ left: `${clip.timeline_start_ms / audio.master_duration_ms * 100}%`, width: `${(clip.timeline_end_ms - clip.timeline_start_ms) / audio.master_duration_ms * 100}%` }} title={`${clip.timeline_start_ms / 1000}s–${clip.timeline_end_ms / 1000}s`} onClick={() => selectClip(clip.id)}><b>{clip.label}</b><small>{clip.timeline_start_ms / 1000}s–{clip.timeline_end_ms / 1000}s</small></button>)}</div></article>})}</section>
    {selected && currentDraft ? <section className="brand-sound-cue-editor"><header><div><span className="section-label">当前声音事件</span><h5>{selected.track.role}</h5></div><button className="brand-fact-delete" disabled={busy} onClick={() => onSave([{ op: 'remove_clip', clip_id: selected.clip.id }])}><Trash2 size={14}/>删除</button></header><label className="wide">声音描述<textarea value={currentDraft.label} onChange={event => updateCurrent({ label: event.target.value })}/><small>使用自然语言描述声音效果；真实 AI 音频模型接入后将据此生成或重新生成该事件。</small></label><div className="brand-sound-cue-controls"><label>开始（ms）<input type="number" min={0} max={audio.master_duration_ms - 1} value={currentDraft.start} onChange={event => updateCurrent({ start: Number(event.target.value) })}/></label><label>结束（ms）<input type="number" min={1} max={audio.master_duration_ms} value={currentDraft.end} onChange={event => updateCurrent({ end: Number(event.target.value) })}/></label><label>事件音量<input type="range" min={-36} max={12} step={1} value={currentDraft.gain} onChange={event => updateCurrent({ gain: Number(event.target.value) })}/><small>{currentDraft.gain} dB</small></label><label>淡入（ms）<input type="number" min={0} max={3000} value={currentDraft.fadeIn} onChange={event => updateCurrent({ fadeIn: Number(event.target.value) })}/></label><label>淡出（ms）<input type="number" min={0} max={3000} value={currentDraft.fadeOut} onChange={event => updateCurrent({ fadeOut: Number(event.target.value) })}/></label></div>{assetURLs[selected.clip.id] ? <audio controls preload="metadata" src={assetURLs[selected.clip.id]}/> : <small>等待此声音事件的项目音频资产</small>}</section> : null}
    <div className="brand-actions">{materialized ? <span className="brand-confirmed"><Check size={14}/>声音资产已就绪</span> : <><button className="primary-button" disabled={busy} onClick={onGenerateAI}>AI 生成音乐与音效</button><button className="secondary-button" disabled={busy} onClick={onMaterialize}>使用开发演示音频</button></>}<button className="secondary-button" disabled={busy || operations.length === 0} onClick={() => onSave(operations)}>保存声音修改</button><button className="primary-button" disabled={busy || !materialized || operations.length > 0 || audio.status === 'preview_queued' || audio.status === 'preview_rendering'} onClick={onRender}>{audio.status === 'preview_queued' || audio.status === 'preview_rendering' ? <><LoaderCircle className="spin" size={14}/>正在混音</> : mixedVideoURL ? '重新生成混音预览' : '生成混音预览'}</button></div>
  </div>
}

function SoundDesignIntentEditor({ plan, disabled, onChange }: { plan: ApiBrandFilmPlan; disabled: boolean; onChange: (changes: Partial<ApiBrandFilmPlan>) => void }) {
  const intent = plan.sound_design_intent ?? { music_direction: plan.music_direction ?? '', sound_effect_focus: ['镜头动作与产品材质', '转场与品牌定格'], source_audio_policy: 'mute' as const, avoid: ['人声'] }
  const update = (changes: Partial<typeof intent>) => onChange({
    music_direction: changes.music_direction ?? intent.music_direction,
    sound_design_intent: { ...intent, ...changes },
  })
  return <section className="brand-sound-intent-editor">
    <header><div><span className="section-label">SOUND DESIGN INTENT</span><h4>声音意图</h4></div><small>只规划音乐、氛围和音效重点；不再要求旁白或口播。</small></header>
    <label>音乐方向<textarea disabled={disabled} value={intent.music_direction} onChange={event => update({ music_direction: event.target.value })}/></label>
    <label>音效重点<textarea disabled={disabled} value={stringListOrEmpty(intent.sound_effect_focus).join('、')} onChange={event => update({ sound_effect_focus: compactLines(event.target.value.split(/[、\n]/)) })}/><small>用“、”或换行分隔，例如：产品材质、镜头转场、品牌定格。</small></label>
    <label>原视频声音<select disabled={disabled} value={intent.source_audio_policy} onChange={event => update({ source_audio_policy: event.target.value as typeof intent.source_audio_policy })}><option value="mute">默认静音</option><option value="optional">需要时可用</option><option value="include">保留并参与混音</option></select></label>
    <label>避免出现<textarea disabled={disabled} value={stringListOrEmpty(intent.avoid).join('、')} onChange={event => update({ avoid: compactLines(event.target.value.split(/[、\n]/)) })}/><small>例如：人声、夸张水花、嘈杂环境声。</small></label>
  </section>
}
...
function EditableList({ title, items, disabled, onChange }: { title: string; items: string[]; disabled: boolean; onChange: (items: string[]) => void }) {
  return <label><h4>{title}</h4><textarea disabled={disabled} value={stringListOrEmpty(items).join('\n')} onChange={event => onChange(event.target.value.split('\n'))}/></label>
}
