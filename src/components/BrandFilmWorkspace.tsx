...
function SoundDesignIntentEditor({ plan, disabled, onChange }: { plan: ApiBrandFilmPlan; disabled: boolean; onChange: (changes: Partial<ApiBrandFilmPlan>) => void }) {
  ...
  return <section className="brand-sound-intent-editor">
    ...
    <label>音效重点<textarea disabled={disabled} value={intent.sound_effect_focus.join('、')} onChange={event => update({ sound_effect_focus: compactLines(event.target.value.split(/[、\n]/)) })}/><small>用“、”或换行分隔，例如：产品材质、镜头转场、品牌定格。</small></label>
    ...
    <label>避免出现<textarea disabled={disabled} value={(intent.avoid ?? []).join('、')} onChange={event => update({ avoid: compactLines(event.target.value.split(/[、\n]/)) })}/><small>例如：人声、夸张水花、嘈杂环境声。</small></label>
  </section>
}
